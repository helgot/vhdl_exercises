...
entity rom is
  port
  (
    up_select, down_select, write, clk	: in  std_logic;
...
architecture rtl of rom is
  ...
  signal count_r            : std_logic_vector(rom_addr_bits-1 downto 0);
...
begin
	...
	rom_address_r <= count_r;

	count_r_update : process(clk) is
		variable up_select_prev   : std_logic := '0';
		variable down_select_prev : std_logic := '0';
	begin
		if rising_edge(clk) then
			if up_select = '1' and up_select_prev = '0' then
				count_r <= std_logic_vector(unsigned(count_r) + 1);
			elsif down_select = '1' and down_select_prev = '0' then
				count_r <= std_logic_vector(unsigned(count_r) - 1);
			end if;			
			up_select_prev := up_select;
			down_select_prev := down_select;
		end if;
	end process;
	rom_update : process(clk) is
	begin
		if rising_edge(clk) then
			leds_out <= rom(to_integer(unsigned(rom_address_r)));
		end if;
		if write = '1' then
		  rom(to_integer(unsigned(rom_address_r))) <= switches_in;
		end if;
	end process;
end architecture;
